...
# ОБНОВЛЕНИЕ — пере-якорение на `bwd-sealed-v2` (2026-07-13)

Клон переведён с `w0-seal-v1` на **`bwd-sealed-v2`** (goml commit `2314cbe`,
*"seal: FP8 bwd v2 (42.35ms E2E / 415.44T proj) — 061 S2v4 dk_new"*).

**Причина:** v1 (dk_new 128 рег) больше не финальная. v2 даёт **−4.4% wall** (42.35 ms nc / 22.206 ms causal,
415.44 T proj) на S2v4-версии dk_new. Это **снимает бывший рассинхрон fingerprint**: reference-бенч уже ждал
`dk_new = 124`, и теперь sealed-ядро действительно 124 — самопроверка проходит естественно.
Даунгрейд бенча 124→128 отвергнут (терять throughput ради старой sealed — неверно).

**Дельта v1→v2 — ровно один файл:**
| файл | v1 | v2 | статус |
|---|---|---|---|
| `fa_bwd_dk_new.cu` | a9f0ded8 (128 рег) | **`25e5e107`** (124 рег, S2v4) | ЗАМЕНЁН |
| fa_bwd_merged_v1.cu | 2bf32ab7 | 2bf32ab7 | без изменений |
| fa_bwd_dq_new.cu | d7a11a3d | d7a11a3d | без изменений |
| fa_bwd_common.cuh | 4407ec9c | 4407ec9c | без изменений |
| fa_bwd_dk.cu | 068d6a4f | 068d6a4f | без изменений |

md5 всех 5 после замены сверены с тегом `bwd-sealed-v2` — **PASS**.
Reference-бенч уже был свежий (EXPECT 124 + патч 070 `dS_T = nullptr`) — не трогали.
Также подтянут `docs/FP8_FA_ROADMAP.md`. `w0-seal-v1` остаётся на remote для старых экспериментов.
