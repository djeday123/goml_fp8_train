// =============================================================================
// FlashAttention v121r-TRAIN — FP8 Forward + LSE output for backward
// =============================================================================
// L-патч на v121r (production, tag v0.1.0).
// Дополнение: записать L = m_i + log(l_i) на Q-строку (LSE, log-sum-exp),
// нужен для backward (Tri Dao Variant 3 recompute).
//
// Изменения vs production v121r:
//   1. Сигнатура: добавлен `float* __restrict__ L_out` (per Q-row buffer,
//      layout [bh, seq_len] row-major). nullptr — производство без L.
//   2. Имя kernel'а: fa96b_kernel → fa96b_train_kernel (символьная изоляция).
//   3. Эпилог: после O-write добавлены 2 строки L-вычисления + 1 ST.E.32
//      на Q-строку. tid==0 гейт — только 1 thread из 4-col-группы пишет.
//      `L = (rmax + log2f(rsexp)) * loge(2)` — конвертация из log2-space
//      kernel'а в natural base (kernel умножает scores на log2(e) = 1.4426,
//      чтобы использовать exp2.approx.f16x2 → log2-space внутри, natural
//      снаружи).
//
// **Не должен изменить байт-в-байт O production-kernel'а.** Только добавляет
// L-output. ptxas регистры — допуск +2-3 (рассчёт: log2f = 1 reg на временное
// значение, FFMA inplace, ST.E.32 без отдельного reg).
//
// Используется только когда фа_forward'у нужен LSE (backward training path).
// =============================================================================
...
__global__ void __launch_bounds__(FA_THREADS, 2)  // R-cost measurement at 2 blocks/SM
    fa96b_train_kernel(
        ...
        float *__restrict__ L_out,  // [bh, seq_len] log-sum-exp per Q-row, may be nullptr
        ...
        int window)  // 0 = no window (full attention or causal). >0 = sliding window.
{
    ...
    __half *Oh = O + bh * hs;
    // L-патч: per-batch_head slice of LSE buffer.
    // nullptr → don't write L (production-path compatible call).
    float *Lh = (L_out != nullptr) ? (L_out + bh * seq_len) : nullptr;
...
#pragma unroll
    ...
    // L-патч: запись LSE ПЕРЕД O-write loop'ом. log2f-инструкции issue'ятся
    // в shadow O-store LSU latency (scheduler interleaves arith vs LSU).
    // rmax[mi][r] хранится в log2-space (scores * log2(e) = 1.4426 на входе
    // softmax, см. line ~422 production: `fs = scale*qk_descale*1.4426`).
    // rsexp[mi][r] = sum exp(s_natural - m_natural) = l_i (base-инвариант).
    // L_natural = m_natural + log(l_natural) = (rmax_log2 + log2(rsexp)) × loge(2).
    // Гейт tid==0 — только 1 thread из 4-col-группы пишет (rmax/rsexp
    // одинаковы у всех 4 col-threads после shfl-reduce на line ~486-489).
    if (Lh != nullptr && tid == 0)
    {
        constexpr float LN2 = 0.6931471805599453f;  // loge(2)
#pragma unroll
        for (int mi_l = 0; mi_l < M_TILES; mi_l++)
        {
            int mr_l = mrb + mi_l * 16;
            int gr0_l = qs + mr_l + gid, gr8_l = gr0_l + 8;
            float L0 = (rmax[mi_l][0] + log2f(rsexp[mi_l][0])) * LN2;
            float L1 = (rmax[mi_l][1] + log2f(rsexp[mi_l][1])) * LN2;
            if (gr0_l < seq_len) Lh[gr0_l] = L0;
            if (gr8_l < seq_len) Lh[gr8_l] = L1;
        }
    }
...
static void cpu_attention_fp8_train_unused_(
